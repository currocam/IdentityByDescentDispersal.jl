...
run(
    `slim -p -s $seed -d NE=$NE -d SD=$SD -d SM=$SM -d OUTPATH="\"$outpath\"" ../../simulations/constant_density.slim`,
)
...
using JLD2
df = df2
@save "constant_density.jld2" local_density dispersal_rate df contig_lengths
