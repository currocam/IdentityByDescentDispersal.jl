...
# Mates tend to live close to each other and their offspring. This results in an inverse correlation between geographical distance and genetic relatedness. The goal of this package is to do demographic inference based on such spatial genetic patterns.

# ## Diffusion approximation of dispersal

# First, let’s provide some intuition on how we model the dispersal of individuals in continuous space.
# From an ecological perspective, we might be interested in modeling the details of single-generation dispersal, such as the mating process or differences in dispersal across individuals and sexes.
# Such details are, however, not generally feasible to estimate from genetic data. Instead, we aim to estimate the mean dispersal rate, i.e. the expected euclidean distance between parent and offspring, across generations.

# This method approximates the spatial movement of genetic material using a diffusion process.
# Let $r(t)$ be the distance between two homologous loci at a time $t$ before the present.
# First, note that when $t=\text{TMRCA}$ (time to the most common recent ancestor), then $r(\text{TMRCA}) = 0$. The distance
# between both loci at the present, $r(0)=r_{\text{obs}}$ (i.e. the distance we can observe), is the sum of the sequence of dispersal events since the time to the most common recent ancestor.

# If dispersal distances are independent and identically distributed, then, by the central-limit theorem, the random variable $r_{\text{obs}}$ converges to a Gaussian distribution with axial variance of $\sigma^2\times \text{TMRCA}$ as $\text{TMRCA}$ grows. Here $\sigma^2$ is the average squared axial parent-offspring distance (i.e. the dispersal rate).

# This result is independent of the details of the single-generation dispersal process, as long as the variance of the dispersal kernel is finite. For example, let’s consider a population where dispersal across a single-generation dispersal can be modeled using a uniform distribution. Below, we visualize the distribution of distances between homologous loci when $t = 1$ (single-generation dispersal) and $t=30$.
# For simplicity, we assume 1-dimensional space. As we can see, the approximation becomes very accurate even
...
# and when dispersal is not correlated across generations and is time-homogeneous (i.e. displacements are independent and identically distributed).
...
# probability of two homologous loci being more than one unit apart given that they coalesced 20 generations ago can
# be calculated from the density of the Gaussian approximation for any $\sigma$.
...
# marginalize across all possible times $[0, \infty)$. Here one can use standard coalescent theory to model the time to coalescence. Roughly speaking, we consider a scenario where two homologous loci *might* coalesce when they
# become very close. The rate with which they coalesce when they are very close is inversely proportional to local effective density $D_e$ (effective number of individuals per unit of space).
# According to this model, the coalescence rate at time $t$ before the present for two homologs that are currently a distance $d$ apart is $p(r(t)=0|r(0)=d) \times \frac{1}{D_e(t)}$, i.e. the rate at which they come close enough times the rate at which they coalesce given that they are close enough.
...
# ## IBD blocks
# The last ingredient of this method is the relationship between identity-by-descent (IBD) blocks
...
# identity-by-descent blocks, as such data are more informative about the recent past than polymorphism data.
#
# We say that a segment of DNA is a shared identity-by-descent block if it has been inherited from a common ancestor
# without being broken up by recombination. A shared identity-by-descent block of length $l$ that finds its common ancestor at time $t$
# has experienced $2t$ meiosis. Therefore, we expect $l$ to decay with $t$.
#
# If we model recombination as a Poisson process and measure genetic distance in Morgans (as is typically done), then
# the probability that a region of length $L$ does not recombine can be obtained from the exponential distribution with rate
# $\exp(-2Lt)$
#
...
# that has not recombined (the $\exp(-2lt)$ term) which is delimited by two recombination events (the $4 t^2$ term) and summing
...
#
# where $\phi(t)$ is the probability density function of the time to the most recent common ancestor. This expression is hard to track
