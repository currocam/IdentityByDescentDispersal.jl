...
Other related software includes `spacetrees` [@osmond] which estimates dispersal rates from inferred genome-wide genealogies and `disperseNN2` [@smith_dispersenn2_2023], a machine learning framework for predicting the displacement distance from genetic variation data. `IdentityByDescentDispersal.jl` differs in the nature of the data it uses for inference, but also in that it allows for flexible model-based inference of the effective density. Moreover, we expect `IdentityByDescentDispersal.jl` to be several orders of magnitude faster than `disperseNN2` for large datasets. For example, finding the MLE of the example dataset (n=100 diploids) takes less than a minute in a single CPU with `IdentityByDescentDispersal.jl`, whereas @smith_dispersenn2_2023 reported an approximate runtime of a week with a GPU training the disperseNN2 model for n=100 diploids.
